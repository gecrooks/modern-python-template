...
def git_init_and_tag():
    ...
    subprocess.check_output("git commit -m 'Initialize from gecrooks-python-template'", shell=True, stderr=subprocess.STDOUT)
    subprocess.check_output("git tag v0.0.0", shell=True, stderr=subprocess.STDOUT)
    subprocess.check_output("pip install -e .", shell=True, stderr=subprocess.STDOUT)
